...
public class ScheduleManActivity extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_schedule_man);
  ...
  /*  alert_Dialog.setMessage(BarCodeNo);
    ...
    public void btn_GetData() {
        ...
        clsDailyVisits.setCustNo("      " + "الرقم" + "        ");
        clsDailyVisits.setCustNm("     " + " الاسم" + "  ");
        clsDailyVisits.setCustType("نوع العميل");
        clsDailyVisits.setCustMobile("صافي الذمة");
        ...
        clsDailyVisits.setBarcode("باركود");
        ...
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(ScheduleManActivity.this);
        String  UserID = sharedPreferences.getString("UserID", "");
        q2 = q2+" and Sman = '"+UserID+"' ";
       /* if (Week_Num.equalsIgnoreCase("2")) {
        ...
        q = "select   no,name ,Address , Note2 ,'' as Mobile  ,ifnull(Latitude,0.0) as Latitude   , ifnull(Longitude,0.0) as Longitude" +
                "  ,ifnull(barCode,-1)  as barCode   " +
                "  ,ifnull( CUST_PRV_MONTH,0) as CUST_PRV_MONTH  , ifnull(CUST_NET_BAL,0) as CUST_NET_BAL , Pay_How ,ifnull( CustType,'****')  as CustType, PAMENT_PERIOD_NO,ifnull(IDN,0) as IDN from Customers_man where  " + q2 +" and flag='1'  ";
...
//32.018105
                    ...
                    if (selected_location.getLatitude() == 0.0 || selected_location.getLongitude() == 0.0 || near_locations.getLatitude() == 0.0 || near_locations.getLongitude() == 0.0) {
                        clsDailyVisits.setNotes("غير محددة");
                    ...
                    String qq = "select Start_Time,End_Time ,ifnull( Closed,-1) as Closed   from SaleManRounds where Tr_Data =  '" + currentDateandTime.toString() + "'" +
                            " And CusNo='" + c1.getString(c1.getColumnIndex("no")) + "' and IDN= '" + c1.getString(c1.getColumnIndex("IDN")) + "'  order by no desc limit 1";
                    ...
                    if (cc != null && cc.getCount() != 0) {
                        ...
                        if (cc.getString(cc.getColumnIndex("Closed")).equals("0")) {
                            clsDailyVisits.setFlq("الجولة مفتوحة");
                        ...
                        cc.close();
                        if (GetDuraton == 1) {
                            clsDailyVisits.setCustAddress(CalculateVisitDuration(clsDailyVisits.getTf().toString(), clsDailyVisits.getTt().toString()));
                        }
                    } else {
                        clsDailyVisits.setFlq("الزيارة لم تتم");
                        clsDailyVisits.setStates("-1");

                        clsDailyVisits.setSetColor("-1");
                        clsDailyVisits.setTf("0:0:0");
                        clsDailyVisits.setTt("0:0:0");
                        clsDailyVisits.setCustAddress("0:0:0");
                    }
    ...
    public String CalculateVisitDuration(String StartTime, String EndTime) {
        ...
        try {
            ...
            Duration = String.valueOf(diffHours) + ":" + String.valueOf(diffMinutes) + ":" + String.valueOf(diffSeconds);
        } catch (Exception ex) {
            Duration = "0:0:0";
        }
        return Duration;
