...
*/
        ...
        items_Lsit.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
                ...
                else if (getArguments().getString("Scr") == "Man") {
                    ((MentnisActivity) getActivity()).Set_Order(po_obj.getCustNo().toString());
                    Exist_Pop();
                }
...
/*        if (getArguments().getString("typ") == "0") {
        ...
        else
        {
         ...
         if(getArguments().getString("Scr") == "Man")
         {
             if (Filter.length()==0) {
                 q = "Select distinct '' as Net_Total, s.OrderNo ,s.Customer as acc ,s.Date as date ,'' as inovice_type, c.name   " +
                         "from  Maint s     left join Customers c on c.no =s.Customer where  UserID='"+sharedPreferences.getString("UserID", "")+"'";
             }
             else {
                 q = "Select  distinct s.Net_Total ,s.OrderNo ,s.acc ,s.Date as date , s.inovice_type,  CASE s.inovice_type WHEN '-1' THEN  c.name ELSE s.Nm END as  name   from  Sal_invoice_Hdr s left join Customers c on c.no =s.acc" +
                         " Where  ifnull(s.doctype,'1')='"+DocType + "' and "+" UserID='"+sharedPreferences.getString("UserID","") +"'" +
