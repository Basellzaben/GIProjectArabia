...
public class PostSalesreturn {
    ...
    private void FillSal_InvAdapter(String OrderNo , String DocType) {
        ...
        query = "  select distinct  ifnull(Damaged,'') as Damaged ,  ifnull(Note,'') as  Note,      ifnull(pod.weight,0) as weight   ,       ifnull(pod.Operand,0) as Operand  ,  pod.bounce_qty, pod.OrgPrice , pod.tax_Amt , pod.total ,Unites.UnitName,  invf.Item_Name, pod.itemno,pod.price,pod.qty,pod.tax ,pod.unitNo  " +
