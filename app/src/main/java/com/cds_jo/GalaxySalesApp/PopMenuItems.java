...
public class PopMenuItems  extends DialogFragment implements View.OnClickListener  {
    ...
    public View onCreateView( LayoutInflater inflater   , ViewGroup container  ,Bundle savestate){
        ...
        form =inflater.inflate(R.layout.layout,container,false);
     ...
     if (v == add) {
        ...
        /* if( dis.getText().toString().length() == 0 ) {
         ...
         /*if (getArguments().getString("Scr") == "po") {
             ((OrdersItems) getActivity()).Save_List(ItemNo, Price.getText().toString(), qty.getText().toString(), tax.getText().toString(), UnitNo, disc_per.getText().toString(), bounce.getText().toString(), str, UnitName, disc_Amt.getText().toString(),"","","","0");

         } else*/

                if (getArguments().getString("Scr") == "RetnQty") {
