...
public class Sal_Inv_return_SearchActivity extends DialogFragment implements View.OnClickListener {
    View form;
    Button add, cancel;
    ListView items_Lsit;
    // TextView itemnm;
...
String str=(String)o;//As you are using Default String Adapter*/
                ...
                if (getArguments().getString("Scr") == "Sal_inv") {
                    if (DocType.equalsIgnoreCase("1")) {
                        ((Sale_ReturnActivity) getActivity()).Set_Order(po_obj.getCustNo().toString());
                        Exist_Pop();
                    }
    ...
    private void FillList(String Filter) {
        ...
        if (Filter.length() == 0) {
            ...
            q = "Select  distinct s.Net_Total ,s.OrderNo ,s.acc ,s.date , s.inovice_type,  CASE s.inovice_type WHEN '-1' THEN  c.name ELSE s.Nm END as  name   from  Sal_invoice _Hdr s left join Customers c on c.no =s.acc" +
