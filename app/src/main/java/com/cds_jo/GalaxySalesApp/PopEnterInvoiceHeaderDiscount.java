...
public class PopEnterInvoiceHeaderDiscount extends DialogFragment implements View.OnClickListener {
    View form;
    NumberPicker np;
    Button btnSave;
...
*/
    ...
    public void onClick(View v) {
        ...
        EditText ed_Total = (EditText) form.findViewById(R.id.ed_Total);
        Double DiscountPercent;
        if (v == btnSave) {
            if (rdoAmt.isChecked()) {
                 DiscountPercent = (SToD(et_Discount.getText() + "") / SToD(ed_Total.getText() + "")) * 100;
            }
            else
            {
                DiscountPercent =Double.parseDouble(et_Discount.getText().toString());
            }
            if(((Sale_InvoiceActivity) getActivity()).discountcheck(DiscountPercent)){
